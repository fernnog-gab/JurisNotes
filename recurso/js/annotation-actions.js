...
/* ================================================
   MÓDULO: AGRUPAMENTO DE PROVAS EM PILHAS (RO)
   ================================================ */
(function () {
    'use strict';
    
    if (!window.TopicsManager) window.TopicsManager = {};

    const _getMestre = (topicoId, parentIndex) => {
        const t = topicos.find(x => x.id === topicoId);
        return t ? t.anotacoes[parentIndex] : null;
    };

    const _gerarIdPilha = () => 'info-grp-' + Date.now().toString(36) + '-' + Math.random().toString(36).slice(2, 8);

    window.TopicsManager.agruparPilhaInfoProcessual = async function (topicoId, parentIndex, correlacionadoIndex) {
        if (correlacionadoIndex == null) return exibirToast('O Card Principal (Mestre) não pode ser empilhado.', 'erro');
        
        const mestre = _getMestre(topicoId, parentIndex);
        if (!mestre || !Array.isArray(mestre.itensCorrelacionados)) return;
        
        const cardAlvo = mestre.itensCorrelacionados[correlacionadoIndex];
        if (!cardAlvo) return;
        if (cardAlvo.pilhaInfoId) return exibirToast('Este item já pertence a uma pilha. Desagrupe primeiro.', 'aviso');

        const compativeis = mestre.itensCorrelacionados
            .map((item, idx) => ({ item, idx }))
            .filter(({ item }) => item.documento === cardAlvo.documento && item.polo === cardAlvo.polo && !item.pilhaInfoId);

        if (compativeis.length < 2) {
            return exibirToast('Não há outros fragmentos compatíveis deste documento para empilhar.', 'aviso');
        }

        const preview = compativeis.slice(0, 5).map(({ item }) => `• ${item.documento} (fl. ${item.pagina || '?'})`).join('\n');
        const resto = compativeis.length - 5;
        const msg = `Serão empilhados ${compativeis.length} fragmentos:\n\n${preview}` + (resto > 0 ? `\n... e mais ${resto} item(ns).` : '') + `\n\nDeseja prosseguir? A ação é reversível.`;
        
        if (!confirm(msg)) return;

        const novaPilhaId = _gerarIdPilha();
        compativeis.forEach(({ item }) => {
            item.pilhaInfoId = novaPilhaId;
            item.nosExpandidos = false; 
        });

        renderizarTopicos();
        if(typeof salvarBackupAutomatico === 'function') salvarBackupAutomatico();
        exibirToast(`🗂️ ${compativeis.length} itens empilhados.`, 'sucesso');
    };

    window.TopicsManager.desagruparPilhaInfoProcessual = function (topicoId, parentIndex, pilhaId) {
        if (!pilhaId) return;
        const mestre = _getMestre(topicoId, parentIndex);
        if (!mestre || !Array.isArray(mestre.itensCorrelacionados)) return;

        const membros = mestre.itensCorrelacionados.filter((i) => i.pilhaInfoId === pilhaId);
        if (membros.length === 0) return;

        if (!confirm(`Desagrupar ${membros.length} fragmentos? Eles voltarão à lista original.`)) return;

        membros.forEach((item) => {
            delete item.pilhaInfoId;
            delete item.nosExpandidos;
        });

        renderizarTopicos();
        if(typeof salvarBackupAutomatico === 'function') salvarBackupAutomatico();
        exibirToast('Pilha de provas desagrupada.', 'sucesso');
    };

    window.TopicsManager.toggleNosCardEmpilhado = function (topicoId, parentIndex, itemId) {
        const mestre = _getMestre(topicoId, parentIndex);
        if (!mestre || !Array.isArray(mestre.itensCorrelacionados)) return;
        
        const item = mestre.itensCorrelacionados.find((i) => i.id === itemId || i.uuid === itemId || String(mestre.itensCorrelacionados.indexOf(i)) === itemId);
        if (!item) return;

        item.nosExpandidos = !item.nosExpandidos;
        renderizarTopicos();
    };
})();