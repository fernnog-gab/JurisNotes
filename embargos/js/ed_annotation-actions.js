...
/* --- INTEGRAÇÃO DO MOTOR DE EDIÇÃO (JURISEDITOR) --- */
window.JurisEditor = {
    _getTextarea: function() {
        return document.getElementById('edit-text-input');
    },
    
    _wrapText: function(prefix, suffix) {
        ...
        if (start === end) {
            if (typeof exibirToast === 'function') exibirToast('Selecione um trecho para formatar.', 'aviso');
            return;
        }

        const trechoSelecionado = texto.substring(start, end);
        
        // Evita aninhamento duplicado se o usuário clicar duas vezes
        if (texto.substring(start - prefix.length, start) === prefix && texto.substring(end, end + suffix.length) === suffix) {
            // Remove a formatação (Toggle off)
            textarea.value = texto.substring(0, start - prefix.length) + trechoSelecionado + texto.substring(end + suffix.length);
            textarea.setSelectionRange(start - prefix.length, end - prefix.length);
        } else {
            // Aplica a formatação (Toggle on)
            textarea.value = texto.substring(0, start) + prefix + trechoSelecionado + suffix + texto.substring(end);
            textarea.setSelectionRange(start + prefix.length, start + prefix.length + trechoSelecionado.length);
        }
        textarea.focus();
    },

    formatar: function(comando) {
        if (comando === 'bold') this._wrapText('**', '**');
        else if (comando === 'italic') this._wrapText('*', '*');
        else if (comando === 'underline') this._wrapText('<u>', '</u>');
    },

    aplicarTamanho: function(classe) {
        const nivel = classe === 'txt-largo-1' ? 1 : 2;
        if (typeof aplicarAumentoFonte === 'function') {
            aplicarAumentoFonte(nivel); // Reaproveita a função segura já existente
        }
    },

    copiarComo: function(tipo) {
        const textarea = this._getTextarea();
        if (!textarea.value.trim()) {
            if (typeof exibirToast === 'function') exibirToast('Não há texto para copiar.', 'aviso');
            return;
        }
        
        // Em um cenário real, 'html' processaria o markdown antes. 
        // Como fallback robusto, mandamos o texto estruturado para a área de transferência.
        navigator.clipboard.writeText(textarea.value).then(() => {
            if (typeof exibirToast === 'function') exibirToast(`Texto copiado para a área de transferência!`, 'sucesso');
        }).catch(() => {
            if (typeof exibirToast === 'function') exibirToast('Erro ao copiar texto.', 'erro');
        });
    }
};
