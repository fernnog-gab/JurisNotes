            ...
            return `
                <div class="${itemWrapperClass}" data-source="${sub.viewSource}">
                    <div class="sub-annotation-card ${bordaFaseClass}">
                        ...
                        ${_gerarBtnRevisaoHtml(activeTabId, topicoIndex, sub.viewSource, sub.localIndex, intencao, isRevisada)}
                    </div>
                </div>`;
        }).join('');
    ...
    function _construirHTML(topico) {
        ...
        topico.anotacoes.forEach(an => {
            ...
            if (teseAtual !== ultimaTese) {
                html += `<div class="outline-h2" style="color: #6a1b9a; margin-top:32px;">⚖️ Óbice / Tese: ${TopicsManager.escaparHTML(teseAtual)}</div>`;
                
                // NOTA ARQUITETURAL: Em AI, a matriz denomina-se diretrizesPorObice
                if (topico.diretrizesPorObice && topico.diretrizesPorObice[teseAtual]) { 
                    const diretrizesVisiveis = topico.diretrizesPorObice[teseAtual].filter(d => d.intencao !== 'nota');
                    diretrizesVisiveis.forEach(dir => {
                        const intencaoSegura = TopicsManager.escaparHTML(dir.intencao || 'DIRETRIZ');
